...
if not GITLAB_TOKEN:
    ...
    print("You can generate one under: User Settings -> Access Tokens (with 'api' scope).")
    print("----------------------------------------------------------------------")
    try:
...
def create_issue(issue_info):
    ...
    if response.status_code in (200, 201):
        ...
        print(f"Successfully created issue: '{issue_info['title']}' (IID: {created_issue['iid']})")
        return created_issue["iid"]
    else:
        print(f"Failed to create issue '{issue_info['title']}': {response.status_code} {response.text}")
        return None
...
def set_estimate(issue_iid, duration):
    ...
    if response.status_code == 200:
        ...
        print(f"  Failed to set estimate for issue #{issue_iid}: {response.status_code} {response.text}")
